...
const App = () => {
            ...
            try {
                ...
                const parseMessages = (rawData) => {
                    if (!Array.isArray(rawData))
                        return [];
                    return rawData.filter((item) => (typeof item === 'object' &&
                        item !== null &&
                        'Datetime' in item &&
                        'Sender' in item &&
                        'Message' in item));
    ...
    return (<div className="app-container">
      ...
      <div className="bookmark-bar">
        {Array.from({ length: chunks }).map((_, index) => (<button key={index} className={`bookmark ${currentChunk === index ? 'active' : ''}`} onClick={() => jumpToChunk(index)}>
